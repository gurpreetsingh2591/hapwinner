...
class MyPastTicketsPage extends StatefulWidget {
  const MyPastTicketsPage({Key? key}) : super(key: key);

  @override
  MyPastTicketsState createState() => MyPastTicketsState();
}

class MyPastTicketsState extends State<MyPastTicketsPage> {
  final GlobalKey<ScaffoldState> _scaffoldKey = GlobalKey<ScaffoldState>();

  bool isLogin = false;
  bool isLoader = false;
  String token = "";
  final myTicketBloc = MyTicketBloc();
  ...
  void initState() {
    ...
    myTicketBloc.add(GetMyPastTicketListData(token: token));
  }

  getTickets(dynamic ticketResponse) {
    var ticketList = ticketResponse['data']['user-all-ticket'];
    if (ticketList != "") {
  ...
  Widget build(BuildContext context) {
    ...
    return BlocProvider(
      create: (context) => myTicketBloc,
      child: Scaffold(
        key: _scaffoldKey,
        body: ColoredSafeArea(
          child: BlocBuilder<MyTicketBloc, CommonState>(
  ...
  Widget buildContestContainer(BuildContext context, Size mq) {
    return Container(
      ...
      child: myTicketList.isNotEmpty?ListView.builder(
        shrinkWrap: true,
        primary: false,
